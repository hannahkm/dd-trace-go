...
// TestPolicyFiles validates that all gateway policy files are well-formed and
// reference the correct protocol fields (POST /, addresses envelope, new
// response fields). This catches drift between the Go handler and the gateway
// policy templates.
func TestPolicyFiles(t *testing.T) {
	...
	// Deprecated paths/fields that must not appear in any policy file.
	deprecated := []string{
		"/v1/request",
		"/v1/response",
		`"action"`,
		`"request_id"`,
		`"status_code"`, // top-level; inside addresses is fine
	}
	...
	for _, dir := range policyDirs {
		...
		for _, entry := range entries {
			allEntries = append(allEntries, struct {
				name string
				path string
			}{entry.Name(), filepath.Join(dir, entry.Name())})
		}
	}
	require.NotEmpty(t, allEntries, "no policy files found in any policy directory")
	...
	for _, entry := range allEntries {
		...
		case ".xml":
			t.Run("xml/"+name, func(t *testing.T) {
				// APIM policy XML files contain C# template expressions (@{...})
				// with generics (e.g., As<byte[]>) that make them invalid strict
				// XML. We validate structure via string checks instead.
				content := string(data)

				// Must have basic XML structure
				assert.Contains(t, content, "<set-url>", "%s missing <set-url> element", name)
				assert.Contains(t, content, "<set-method>POST</set-method>", "%s missing POST method", name)
				assert.Contains(t, content, "<set-body>", "%s missing <set-body> element", name)

				// Must reference POST / endpoint (not the old /v1/* paths)
				assert.NotContains(t, content, "/v1/request", "%s still references /v1/request", name)
				assert.NotContains(t, content, "/v1/response", "%s still references /v1/response", name)

				// Must use new response field names
				assert.Contains(t, content, `["block"]`, "%s missing block field reference", name)

				// Inbound policies must extract request-id from Phase 1 response
				if strings.Contains(name, "inbound") || strings.Contains(name, "full") {
					assert.Contains(t, content, `["request-id"]`, "%s missing request-id field reference", name)
				}

				// Must use addresses envelope in set-body
				assert.Contains(t, content, "addresses", "%s missing addresses envelope", name)

				// Must not reference old action/status_code/body top-level fields
				assert.NotContains(t, content, `["action"]`, "%s still references old action field", name)
			})
		...
		case ".json":
			t.Run("json/"+name, func(t *testing.T) {
				...
				require.NoError(t, json.Unmarshal(data, &policy), "%s is not valid JSON", name)

				// URL must be POST / (not /v1/*)
				url, _ := policy["url"].(string)
				assert.NotContains(t, url, "/v1/", "%s URL still references /v1/ path", name)
				assert.True(t, len(url) > 0 && url[len(url)-1] == '/', "%s URL should end with /", name)

				// Method must be POST
				assert.Equal(t, "POST", policy["method"], "%s method should be POST", name)

				// Body template must contain addresses envelope
				body, _ := policy["body"].(string)
				assert.Contains(t, body, `"addresses"`, "%s body missing addresses envelope", name)

				// Body template must contain gateway field for Boomi
				if strings.Contains(name, "boomi") {
					assert.Contains(t, body, `"gateway"`, "%s missing gateway field", name)
					assert.Contains(t, body, `"boomi"`, "%s missing boomi gateway value", name)
				}

				// Variables must not reference old field names
				content := string(data)
				for _, dep := range deprecated {
					if dep == `"status_code"` {
						// status_code inside addresses is fine; check it's not at top level in jsonPath
						assert.NotContains(t, content, "$.status_code", "%s references deprecated top-level $.status_code", name)
						continue
					}
					assert.NotContains(t, content, dep, "%s references deprecated %s", name, dep)
				}
			})

		case ".groovy":
			t.Run("groovy/"+name, func(t *testing.T) {
				// Groovy files: just verify no deprecated path references
				content := string(data)
				assert.NotContains(t, content, "/v1/request", "%s references /v1/request", name)
				assert.NotContains(t, content, "/v1/response", "%s references /v1/response", name)
			})
		}
	}
}
